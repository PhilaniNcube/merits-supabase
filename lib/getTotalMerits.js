...
const getTotalMerits = async () => {
  let { data, error } = await supabaseClient.rpc("my_total_merits");
...
export const getSportsMerits = async () => {
  let { data, error } = await supabaseClient.rpc("get_sports_merits");
...
export const getSocialMerits = async () => {
  let { data, error } = await supabaseClient.rpc("get_social_merits");
...
export const getAcademicMerits = async () => {
  let { data, error } = await supabaseClient.rpc("get_academic_merits");
...
export const getMyMerits = async () => {
  let academic = await supabaseClient.rpc("get_academic_merits");
  let social = await supabaseClient.rpc("get_social_merits");
  let sports = await supabaseClient.rpc("get_sports_merits");
  ...
  return [
    ...
    { item: "academic", count: academic.data, color: "#fca0ba" },
  ];
};
